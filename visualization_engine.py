...
class STLPlotlyVisualizer:
    ...
    def visualize_from_code(self, cadquery_code: str, title: str = "CAD Model", job_id: str = None) -> Tuple[Optional[go.Figure], Optional[str], Optional[str], Optional[str]]:
        """Complete pipeline: CADQuery code -> execution -> STL -> Plotly -> HTML with 3-attempt retry"""
        max_attempts = 3
        last_error = None
        
        for attempt in range(max_attempts):
            try:
                print(f"🔄 Attempt {attempt + 1}/{max_attempts}: Executing CADQuery code...")
                
                # Execute code
                cq_object, exec_error = self.execute_cadquery_code(cadquery_code)
                if exec_error:
                    last_error = f"Code execution failed: {exec_error}"
                ...
                # Create visualization with consistent naming
                if job_id:
                    stl_filename = f"cad_model_{job_id}.stl"
                    step_filename = f"cad_model_{job_id}.step"
                else:
                    timestamp = int(time.time())
                    stl_filename = f"cad_model_{timestamp}.stl"
                    step_filename = f"cad_model_{timestamp}.step"
                
                # Export STL with specific filename
                stl_path, stl_error = self.export_to_stl(cq_object, stl_filename)
                if stl_error:
                    last_error = f"STL export failed: {stl_error}"
                    print(f"❌ Attempt {attempt + 1} STL failed: {last_error}")
                    continue
                
                # Export STEP with specific filename
                step_path, _ = self.export_to_step(cq_object, step_filename)
                
                # Read STL and create visualization
                vertices, faces, read_error = self.read_stl_file(stl_path)
                if read_error:
                    last_error = f"STL reading failed: {read_error}"
                    print(f"❌ Attempt {attempt + 1} STL read failed: {last_error}")
                    continue
                
                # Create Plotly figure
                fig = self.create_plotly_visualization(vertices, faces, title)
                
                # ALWAYS save HTML file to dedicated folder with consistent naming
                html_output_dir = Path("output_htmls")
                html_output_dir.mkdir(exist_ok=True)
                
                if job_id:
                    html_filename = f"cad_model_{job_id}.html"
                else:
                    timestamp = int(time.time())
                    html_filename = f"cad_model_{timestamp}.html"
                ...
                print(f"✅ Complete pipeline successful on attempt {attempt + 1}!")
                print(f"Files created: STL={stl_path}, HTML={html_path_str}")
                return fig, stl_path, None, html_path_str
                
            except Exception as e:
                last_error = f"Complete pipeline failed: {str(e)}"
                print(f"❌ Attempt {attempt + 1} failed: {last_error}")
